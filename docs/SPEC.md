...
**Nazwy butli i dwie kolumny.** Marką butli jest jej materiał — `Stal` albo `Alu` (nie „Aluminium": dłuższe o 6 znaków, a nic nie dodaje). Twinsety nie mają osobnej marki „Twinset", bo `2×` w nazwie mówi to samo krócej; materiał trzymają w marce, jak butle pojedyncze. Sam skrót nie wystarczył, żeby kafelki stanęły parami: kontener ma 324 px, więc dwa mieszczą się dopiero przy ≤159 px, a nazwa butli ma 147–221 px. Dlatego rzędy butli i stage'ów dostały `.chips.cols2` — **siatkę dwóch kolumn** zamiast zawijania — i zeszły z dziesięciu wierszy (410 px) do sześciu (274 px). Trzy najdłuższe nazwy (seria Alu S) łamią się w kafelku na dwie linie; litrów z nich nie usuwamy, bo to aplikacja o objętości butli. Dopisek `(stage)` w nazwach stage'ów zostaje mimo etykiety rzędu: bez niego „Alu S80 11,1 l / 207 bar" istniałoby dwa razy, co łapie test na duplikaty nazw. Zwykłe chipy zestawu zostają zawijaną listą — mają ikonę własny/wypożyczony i bardzo różne długości.
...
**Co zmienia `BRAND`.** Nagłówek (logo centrum, pod nim mała nazwa aplikacji — `brandHeader()`, raz przy starcie; z `logoDark` dwa obrazki, a widoczny wybierają tokeny `--logo-l`/`--logo-d` z trzech bloków `:root`, więc działa i przy motywie „jak w telefonie”, i wymuszonym — `brandLogo()`; `logoPlate: true` zamiast drugiej wersji kładzie oryginalne logo na jasnej plakietce, tokenem `--logo-plate`), powitanie w kreatorze (logo i „Aplikację udostępnia …”), ikona w instrukcji instalacji (`homeIcon()`), kolory (cztery tokeny w **wszystkich trzech** blokach `:root`, `brandTokensCss()`), domowy akwen nowego nurka (`BRAND.site` → `emptyDiver()`), karta centrum z kontaktem w Profilu (`brandCard()`) i aktualności. Manifest, tytuł strony i nazwa pod ikoną na iPhonie są osobne dla każdego centrum.
