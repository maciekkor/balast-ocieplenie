...
**Akwen z lokalizacji telefonu.** Na karcie planu aplikacja pyta raz: „Ustawiać akwen po Twojej lokalizacji?". Odpowiedź siedzi w `S.geo` (`'on'` / `'off'`; brak = jeszcze nie pytaliśmy), a odmowa w samym telefonie też zapisuje `'off'`, żeby nie pytać w kółko. Przy `'on'` `locateSite()` pyta `navigator.geolocation` przy starcie aplikacji i po tapnięciu „Najbliższy akwen", po czym wybiera **najbliższy akwen zwykłym dystansem** (`nearestSite()`, do 500 km) i mówi, który i z jakiej odległości.
