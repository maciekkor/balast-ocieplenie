...
- **Akweny:** preset ma listę nurkowisk `pts` (do 10 punktów) albo sam `lat`/`lon`, plus promień `r`; odległość liczy `siteDistKm()` do najbliższego punktu, więc promienie są małe. `matchSite()` (import) wybiera akwen o najmniejszym `km / r`, `nearestSite()` (przycisk, lista) po prostu najbliższy. Współrzędnych nie ma w UI — `migrate()` bierze `lat`, `lon`, `r` i `pts` zawsze z presetu.
