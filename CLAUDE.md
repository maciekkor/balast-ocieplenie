...
- **Edytor sprzętu:** pozycja z katalogu ma do zmiany tylko własność (przyciski Mój/Wypożyczony), rozmiar (kafelki) i rok zakupu (sam dla „Mój”). Parametry `p` edytuje się wyłącznie w pozycjach własnych — nie dodawaj pól `p` do edytora katalogowego.
...
- **Kreator kończy się sprzętem:** krok 4 wypuszcza dalej dopiero z pianką/suchym, butlą i kamizelką/skrzydłem (`WIZ_NEED`). Pierwsze uruchomienie to `freshState()` (pusta szafa), a `seedState()` służy już tylko „Wczytaj przykład”.
