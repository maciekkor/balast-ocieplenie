...
- **Zasolenie:** wyporność w katalogu jest podana **dla morza 1,025** — tam jest punkt kalibracji. Każda nowa kategoria licząca wyporność ze stałej `p.b` musi przejść przez `atRho()` (`src/model.js`), inaczej akwen nie wpływa na jej wynik. Objętość bierze `dispVol()`: `p.vd`, potem `p.mass`, na końcu typowa z `VDISP`.
